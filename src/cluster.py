...
print(f'embedding shape(임베딩): {embeddings.shape}')
...
print(f'cluster#(클러스터 수): {n_clusters}')
print(f'noise point(노이즈 포인트): {(labels == -1).sum()}')

print('\n=== seq sample for each cluster(클러스터별 서열 샘플) ===')
...
# with seq
print('\n=== seq for each cluster(클러스터별 서열 내용) ===')
seqs = {}
from pathlib import Path
...
for label in sorted(set(labels)):
    if label == -1 or label == 0:  # 0은 너무 많아서 스킵, too many 0s
        continue
    mask = np.where(labels == label)[0]
    print(f'\nCluster {label} ({len(mask)}개):')
    for idx in mask[:3]:
